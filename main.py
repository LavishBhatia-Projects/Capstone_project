...
def predict_single_stock(symbol):
    ...
    try:
        ...
        data_file = get_historical(symbol)
        if not data_file:
            return None
        ...
        pred_price, error = ARIMA_ALGO(df, symbol, quick_mode=True)
        if pred_price == 0:
            return None
...
def get_market_status():
    ...
    for key, ticker in indices.items():
        try:
            # Using fast_info or download
            t = yf.Ticker(ticker)
            # data = t.history(period="1d") # slower
            # Using fast_info
            
            # Using download for reliability on indices sometimes
            df = yf.download(ticker, period="2d", progress=False)
            if len(df) >= 1:
                price = df['Close'].iloc[-1]
                prev = df['Close'].iloc[-2] if len(df) > 1 else df['Open'].iloc[-1]
                if isinstance(price, pd.Series): price = price.iloc[0] # Handle weird df shape
...
def update_trending_stocks():
    ...
    Prioritizes 'static/nse_candidates.json' if available.
    Falls back to Yahoo Finance Trending scaping.
    Runs ARIMA model filtering by volume and score.
    """
    try:
        print("Trending Update: Starting...")
        candidates = []
        source_name = "Yahoo Trending"
        
        # 1. Check for NSE Candidates file first
        nse_candidates_path = os.path.join(STATIC_DIR, "nse_candidates.json")
        if os.path.exists(nse_candidates_path):
            try:
                with open(nse_candidates_path, 'r') as f:
                    candidates = json.load(f)
                if candidates:
                    source_name = "NSE Most Active"
                    print(f"Loaded {len(candidates)} candidates from {nse_candidates_path}")
            except Exception as e:
                print(f"Error loading NSE candidates: {e}")

        # 2. Fallback to Yahoo Scraping if no candidates yet
        if not candidates:
            try:
                ...
                if resp.status_code == 200:
                    pattern = r'"symbol":"([A-Z0-9.-]+)"'
                    all_symbols = list(set(re.findall(pattern, resp.text)))
                    # Filter
                    clean_symbols = [s for s in all_symbols if "=" not in s and "^" not in s]
                    # Filter by Volume > 50k
                    for sym in clean_symbols:
                        try:
                            vol = yf.Ticker(sym).fast_info.get('last_volume', 0)
                            if vol > 50000:
                ...
                print(f"Trending Update: Scraping error: {e}")
                
        # 3. Final Fallback if everything failed
        if not candidates:
             candidates = ['NVDA', 'TSLA', 'AMD', 'AAPL', 'MSFT', 'AMZN', 'GOOG', 'RELIANCE.NS', 'TCS.NS']
             source_name = "Default Fallback"

        # Limit to Top 7 as requested
        candidates = candidates[:7]

        print(f"Trending Update: Running ARIMA on {len(candidates)} symbols from {source_name}")
        
        results = []
        # 4. Sequential Execution (fix for yfinance overlapping data issue)
        ...
        print(f"Trending Update: Complete. Saved {len(top_10)} stocks.")
...
def index():
    ...
    return render_template("index.html", trending_stocks=trending_stocks)
...
def get_historical(quote):
    ...
    same ticker with the ".NS" suffix (NSE). The downloaded CSV is written to a
    filename-safe name and the function returns that base name (without .csv) on
    success, or False on failure.
    """
    end = datetime.now()
    start = end - timedelta(days=1440)  # 4 years

    # Build candidate symbols: try exact user input first, then a .NS fallback
    candidates = [quote]
    # If user gave a plain alphanumeric ticker (no '.', not an index '^'), try .NS
    if "." not in quote and not quote.startswith("^") and re.match(r"^[A-Za-z0-9_-]+$", quote):
        candidates.append(f"{quote}.NS")
    ...
    for sym in candidates:
        try:
            ...
            # sanitize filename (remove ^ from index symbols, replace slashes)
            safe_name = sym.replace("^", "").replace("/", "-")
            data.to_csv(f"{safe_name}.csv", index=False)
            if sym != quote:
                print(f"Resolved input '{quote}' -> '{sym}' and saved as {safe_name}.csv")
            return safe_name
        except Exception as e:
    ...
    try:
        ...
        if quick_mode:
            try:
                ...
                pred = forecast.iloc[0] if hasattr(forecast, "iloc") else float(forecast)
                return round(pred, 2), 0
            except Exception as e:
                # Fallback to simple mean if ARIMA fails
                return round(df_close["Close"].iloc[-1], 2), 0
        ...
        fig.autofmt_xdate()
        plt.tight_layout()
        plt.savefig(os.path.join(STATIC_DIR, f"ARIMA_{quote}.png"), bbox_inches="tight", dpi=120)
        plt.close(fig)
    ...
    try:
        ...
        model = Sequential(
            [
                ...
                BatchNormalization(),
                # Second LSTM layer
                LSTM(32, return_sequences=False, kernel_regularizer=l2(0.001), recurrent_regularizer=l2(0.001)),
                Dropout(0.2),
                BatchNormalization(),
        ...
        history = model.fit(
            ...
            callbacks=callbacks,
            verbose=1,  # Show epoch progress (1=progress bar, 2=one line per epoch)
        ...
        try:
            ...
            if len(actual_original) > 0:
                ...
                ax.grid(alpha=0.25)
            plt.tight_layout()
            plt.savefig(os.path.join(STATIC_DIR, f"LSTM_{quote}.png"), bbox_inches="tight", dpi=120)
            plt.close(fig)
        except Exception:
            pass
...
def LIN_REG_ALGO_MULTIVAR(df, quote, ratio_feat: pd.DataFrame):
    ...
    try:
        ...
        try:
            ...
            ax.grid(alpha=0.25)
            plt.tight_layout()
            plt.savefig(os.path.join(STATIC_DIR, f"LR_{quote}.png"), bbox_inches="tight", dpi=120)
            plt.close(fig)
        except Exception as e:
...
def run_full_analysis(quote):
    ...
    try:
        ...
        # resolved is the safe filename base returned by get_historical (e.g., 'RELIANCE.NS' or 'AAPL')
        company_name = resolve_company_name(resolved) if resolved else None
        # pass a tuple (symbol, company_name) so fetch_news functions can prefer the name
        news_items = fetch_news((resolved, company_name), max_items=25, days_lookback=7)
        agg_sentiment, detailed_sentiment = score_sentiment(news_items)
        plot_sentiment_bars(detailed_sentiment, quote)

        # Recommendation
        print(f"DEBUG: today_close={today_close}, mean_forecast={mean_forecast}")
        ...
        latest_ratios = {
            c: round(float(ratio_features.iloc[-1][c]), 4) if c in ratio_features.columns and pd.notnull(ratio_features.iloc[-1][c]) else None
...
def top_stocks():
    ...
    bullish_stocks = []
    print("\n" + "=" * 80)
    print(f"STARTING TOP STOCKS SCREENER for {len(tickers)} tickers...")
    print("This may take several minutes...")
    print("=" * 80)

    for ticker in tickers:
        # Run the full analysis for each ticker
        results = run_full_analysis(ticker)
        if results.get("error"):
            print(f"Screener: skipped {ticker} due to error: {results.get('message')}")
            continue
        ...
        # Calculate potential gain for filtering/sorting
        try:
            ...
            print(f"Error calculating gain for {stock.get('quote','?')}: {e}")
            stock["potential_gain_pct"] = 0
        
        # New details from the analysis results
        stock["error_lr"] = results.get("error_lr", 0)
        stock["forecast_set"] = results.get("forecast_set", []) # 7-day forecast
        
        # Filter logic: simple threshold
        # e.g. require positive gain OR strong buy decision
        # RELAXED: Show all results to user, let them decide
        # if stock["potential_gain_pct"] > 0.5 or stock["decision"] == "BUY":
    ...
    # 3. Sort the list: highest potential gain first
    bullish_stocks.sort(key=lambda x: x.get("potential_gain_pct", 0), reverse=True)

    # 4. Pass the top results to template and also echo the tickers used
    return render_template("top_stocks.html", top_stocks=bullish_stocks[:10], input_tickers=", ".join(tickers))
